...
public class AddressablesManager : BaseSingleton<AddressablesManager>
{
    ...
    // 资源缓存字典
    readonly Dictionary<string, AssetInfo> resDic = new();
    ...
    {
        foreach (var pair in resDic)
        {
            string key = pair.Key;
            var info = pair.Value;

            Debug.Log($"[Addressables Clear] Release -> {key}  RefCount:{info.RefCount}");
        ...
        Debug.Log("[Addressables Clear] 开始卸载未使用资源");

        var handle = Resources.UnloadUnusedAssets();

        handle.completed += (_) =>
        {
            Debug.Log("[Addressables Clear] 所有资源清理完成");
